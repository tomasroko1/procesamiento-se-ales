...
from hc3.viz import plot_isi_hist, plot_lfp_trace, plot_psd
...
def main():
    ...
    try:
        lfp = load_lfp(
            session, channel_id=lfp_chan, duration=10.0
        )  # Load 10s for trace
        plot_lfp_trace(
            lfp, title=f"Raw LFP (Ch {lfp_chan})", save_path=figures_dir / "lfp_raw.png"
        )
        ...
        plot_lfp_trace(
            lfp_theta,
            title="Theta (4-12Hz) LFP",
            save_path=figures_dir / "lfp_theta.png",
        )
...
"""
    for row in spike_stats:
        report_md += (
            f"![Unit {row['Cluster']} ISI](figures/unit_{row['Cluster']}_isi.png)\n"
        )
