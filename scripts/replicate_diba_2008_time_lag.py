...
from hc3.io import load_behavior, load_session_metadata, load_spikes
...
def main():
    ...
    if beh.empty:
        logger.warning(
            "No behavior data (.whl) found. Cannot correlate with position. Running CCG only."
        )
    ...
    bin_centers, counts = compute_ccg(
        u1.spike_times, u2.spike_times, bin_size_s=0.005, window_s=0.5
    )
    ...
    if not beh.empty:
        ...
        for u in units:
            ...
            rate = (
                spike_hist / occupancy * (1 / 0.0390625)
            )  # approx rate validation needed
