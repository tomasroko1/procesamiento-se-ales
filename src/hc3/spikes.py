...
from .schema import Unit, LFP

def compute_spike_phases(unit: Unit, lfp_phase: np.ndarray, lfp_timestamps: np.ndarray) -> np.ndarray:
    """
    ...
    # Check bounds
    valid_spikes = (unit.spike_times >= lfp_timestamps[0]) & (unit.spike_times <= lfp_timestamps[-1])
    spike_times_valid = unit.spike_times[valid_spikes]
    
    unwrapped_phase = np.unwrap(lfp_phase)
    
    spike_phases_unwrapped = np.interp(spike_times_valid, lfp_timestamps, unwrapped_phase)
    spike_phases = (spike_phases_unwrapped + np.pi) % (2 * np.pi) - np.pi # Wrap to -pi to pi
    
    return spike_phases

def compute_ccg(train1: np.ndarray, train2: np.ndarray, bin_size_s: float = 0.001, window_s: float = 0.5) -> Tuple[np.ndarray, np.ndarray]:
    """
