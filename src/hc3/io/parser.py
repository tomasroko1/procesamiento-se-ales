...
def load_lfp(session: Session, channel_id: Optional[int] = None, duration: Optional[float] = None) -> LFP:
    """
    ...
    return LFP(
        data=data,
        sampling_rate=session.lfp_sampling_rate,
...
def load_spikes(session: Session, min_cluster_id: int = 2) -> List[Unit]:
    ...
    for shank_id in session.shank_map.keys():
        ...
        if len(spike_times_samples) != len(cluster_ids):
            logger.error(f"Mismatch in {session.id} shank {shank_id}: res {len(spike_times_samples)} vs clu {len(cluster_ids)}")
            continue
...
def load_behavior(session: Session) -> pd.DataFrame:
    ...
    try:
        # Load data
        df = pd.read_csv(whl_file, sep=r'\s+', names=['x1', 'y1', 'x2', 'y2'], header=None)
        ...
        # Smooth speed
        df['speed'] = df['speed'].fillna(0).rolling(window=int(fs_beh/2), center=True).mean()
